...
def cell_span(tokens, start, end):
    attr = " ".join(str(token) for token in tokens[start:end])
    return span_values(attr)


def occupancy_badness(tokens):
    rows = []
    for row in row_segments(tokens):
        cells = []
        for start, end, _cell_has_span in iter_cells_in_row(row):
            row_span, col_span = cell_span(tokens, start, end)
            cells.append((row_span, col_span))
        rows.append(cells)
    if not rows:
        return 0.0

    widths = row_widths(tokens)
    target_width = modal_width(widths)
    bad = float(sum(abs(width - target_width) for width in widths if width > 0))
    occupied = {}
    max_width = max([target_width] + widths + [1])

    for r, cells in enumerate(rows):
        c = 0
        for row_span, col_span in cells:
            while (r, c) in occupied:
                c += 1
            if c + col_span > max_width:
                bad += float(c + col_span - max_width)
            for rr in range(r, min(len(rows), r + max(1, row_span))):
                for cc in range(c, c + max(1, col_span)):
                    if (rr, cc) in occupied:
                        bad += 2.0
                    occupied[(rr, cc)] = True
            if r + row_span > len(rows):
                bad += float(r + row_span - len(rows))
            c += max(1, col_span)
    return bad


def shape_badness(tokens):
    return occupancy_badness(tokens)
...
def rerank_tokens(decoded, structure_probs_batch, post_process_class, topk, shape_lambda, pattern_mode=False):
    ...
    plain_td_idx = post_process_class.dict.get("<td></td>")
    open_td_idx = post_process_class.dict.get("<td")
    close_td_idx = post_process_class.dict.get("</td>")
    gt_idx = post_process_class.dict.get(">")
    if not attr_indices:
    ...
    for start, end, cell_has_span in iter_cells(tokens):
        ...
        if pattern_mode and plain_td_idx is not None:
            raw_idx = raw_indices[start]
            old_logprob = sum(
                math.log(max(float(structure_probs_batch[raw_indices[p]][char_indices[p]]), eps))
                for p in range(start, end)
            )
            candidate = list(tokens)
            candidate[start:end] = ["<td></td>"]
            candidate_bad = shape_badness(candidate)
            candidate_logprob = base_logprob - old_logprob + math.log(
                max(float(structure_probs_batch[raw_idx][plain_td_idx]), eps)
            )
            candidate_score = candidate_logprob - shape_lambda * candidate_bad
            if candidate_score > best_score:
                best_score = candidate_score
    ...
    if pattern_mode and all(idx is not None for idx in [open_td_idx, close_td_idx, gt_idx]):
        for start, end, cell_has_span in iter_cells(tokens):
            if cell_has_span:
                continue
            raw_idx = raw_indices[start]
            old_char_idx = char_indices[start]
            old_logprob = math.log(max(float(structure_probs_batch[raw_idx][old_char_idx]), eps))
            for prob, attr_idx, attr_token in top_attr_alternatives(
                post_process_class.character,
                structure_probs_batch[raw_idx],
                attr_indices,
                topk,
                ...
                candidate = list(tokens)
                candidate[start:end] = ["<td", attr_token, ">", "</td>"]
                candidate_bad = shape_badness(candidate)
                candidate_logprob = base_logprob - old_logprob + math.log(
                    max(float(structure_probs_batch[raw_idx][open_td_idx]), eps)
                ) + math.log(max(prob, eps)) + math.log(
                    max(float(structure_probs_batch[raw_idx][gt_idx]), eps)
                ) + math.log(max(float(structure_probs_batch[raw_idx][close_td_idx]), eps))
                candidate_score = candidate_logprob - shape_lambda * candidate_bad
                if candidate_score > best_score:
                    best_score = candidate_score
                    best_tokens = candidate
...
class GatedCellPatternRerankTEDSMetric:
    def __init__(
        self,
        post_process_class,
        topk=3,
        ...
        self.gate_badness = gate_badness
        self.fp = None
        if diagnostics_path:
    ...
    def __call__(self, pred_label, batch=None, *args, **kwargs):
        ...
        for batch_idx, gt_tokens_raw in enumerate(gt_structures):
            ...
            base_badness = shape_badness(pred_tokens)
            if base_badness >= self.gate_badness:
                ...
                reranked_tokens, changes = pred_tokens, 0
            gt_tokens = [
                token
...
def main():
    ...
    parser.add_argument("--gate_badness", type=float, default=0.0)
    ...
    logger.info("phase9d gated cell-pattern rerank structure TEDS eval ***************")
    logger.info("topk:%s", args.topk)
    logger.info("shape_lambda:%s", args.shape_lambda)
    logger.info("gate_badness:%s", args.gate_badness)
