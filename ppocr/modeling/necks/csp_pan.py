...
__all__ = ["CSPPAN", "CSPPAN_YOLOHeader"]
...
class CoordinateAttentionLite(nn.Layer):
    """Lightweight coordinate attention for table header/cell localization.

    YOLO-inspired migration: small financial header/cell regions benefit from
    preserving horizontal/vertical positional cues instead of only channel cues.
    """

    def __init__(self, channels, reduction=16, act="hard_swish"):
        super().__init__()
        hidden = max(8, channels // reduction)
        self.conv1 = ConvBNLayer(channels, hidden, 1, act=act)
        self.conv_h = nn.Conv2D(hidden, channels, 1)
        self.conv_w = nn.Conv2D(hidden, channels, 1)

    def forward(self, x):
        h, w = x.shape[2], x.shape[3]
        x_h = F.adaptive_avg_pool2d(x, output_size=[h, 1])
        x_w = F.adaptive_avg_pool2d(x, output_size=[1, w]).transpose([0, 1, 3, 2])
        y = paddle.concat([x_h, x_w], axis=2)
        y = self.conv1(y)
        y_h, y_w = paddle.split(y, [h, w], axis=2)
        y_w = y_w.transpose([0, 1, 3, 2])
        a_h = F.sigmoid(self.conv_h(y_h))
        a_w = F.sigmoid(self.conv_w(y_w))
        return x * a_h * a_w


class CSPPAN_YOLOHeader(CSPPAN):
    """CSPPAN variant with YOLO-style high-resolution header/cell fusion.

    Migrated ideas:
    - P2/small-object route: preserve the highest-resolution table feature and
      propagate it to all PAN outputs.
    - Multi-scale fusion: inject the P2 route into P3/P4/P5-like outputs.
    - Attention: coordinate attention strengthens row/column positional cues.

    The output interface is unchanged, so existing SLAHead, losses, labels,
    dataset innovations, and Mamba/Mabe modules remain untouched.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.p2_route = ConvBNLayer(out_channels, out_channels, 1, act=act)
        self.p2_mix = nn.LayerList(
            [
                CSPLayer(
                    out_channels * 2,
                    out_channels,
        ...
        self.coord_att = nn.LayerList(
            [CoordinateAttentionLite(out_channels, act=act) for _ in range(len(in_channels))]
        )

    def forward(self, inputs):
        outs = list(super().forward(inputs))
        if not self.p2_fusion:
            return tuple(outs)
        p2 = self.p2_route(outs[0])
        fused_outs = []
        for idx, out in enumerate(outs):
            p2_resized = F.upsample(p2, size=out.shape[2:4], mode="nearest")
            mixed = self.p2_mix[idx](paddle.concat([out, p2_resized], axis=1))
            if self.use_coord_att:
                mixed = self.coord_att[idx](mixed)
            fused_outs.append(mixed)
